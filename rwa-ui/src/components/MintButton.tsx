...
import { TBTC } from '@bricktowers/tbtc-contract';
import {
...
type BrickTowersCoinContract = TBTC.Contract<BrickTowersCoinPrivateState, TBTC.Witnesses<BrickTowersCoinPrivateState>>;
...
export const MintButton: React.FC<MintButtonProps> = ({ logger, onMintTransaction }) => {
  ...
  const onMint: () => Promise<void> = async () => {
    ...
    setMinting(true);
    try {
      const contract: BrickTowersCoinContract = new TBTC.Contract({});
