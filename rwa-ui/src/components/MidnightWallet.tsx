...
interface MidnightWalletState {
  ...
  zkConfigProvider: ZKConfigProvider<RwaCircuitKeys>;
  proofProvider: ProofProvider<RwaCircuitKeys>;
...
export const MidnightWalletProvider: React.FC<MidnightWalletProviderProps> = ({ logger, children }) => {
  ...
  const privateStateProvider: PrivateStateProvider<string, RwaPrivateState> = useMemo(
    () =>
  ...
  const zkConfigProvider = useMemo(
    () =>
      new CachedFetchZkConfigProvider<RwaCircuitKeys>(window.location.origin, fetch.bind(window), providerCallback),
    [],
  );
    ...
    if (walletAPI) {
      return {
        ...
        balanceTx(tx: UnbalancedTransaction, newCoins: CoinInfo[]): Promise<BalancedTransaction> {
          ...
          console.log('Balancing transaction with Midnight Lace wallet', tx, newCoins);
